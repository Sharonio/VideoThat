...
def load_videoclips(paths_to_videoclips):
    ...
    for i in paths_to_videoclips:
        v = mpy.VideoFileClip(str(i)).without_audio()
        videoclips[i.stem] = v
...
def assemble_video(videoclips, chosen_scenes, input_song):
    ...
    for index, row in chosen_scenes.iterrows():
        ...
        final_visuals.append(v.subclip(*v_time)
                              ...
                              .set_start(row.position))   #TODO: we force height to be even, this is all hardcoded :()
    
    # visuals & audio:
    #final_video = mpy.concatenate_videoclips(final_visuals, method='compose')
    final_video = mpy.CompositeVideoClip(final_visuals)
    final_video.audio = input_song

    return final_video

def save_video(final_video):
    final_video.write_videofile("videoThatGlobal.mp4")
    return

def main():
    chosen_scenes_path = 'sandbox/chosen_global.pkl'
